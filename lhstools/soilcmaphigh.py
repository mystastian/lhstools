...
class SoilCMapHigh(Benchmark):
    ...
    def get_sim(self,memberid):
        ...
        sim=sim.where(np.abs(sim.LATITUDE)>=45)
        luarea=xr.open_dataset(fnmember).lu_area
        luarea=luarea.where(np.abs(sim.LATITUDE)>=45)
        #Time constraint
