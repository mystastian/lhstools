...
class BiomassLuyssaert(Benchmark):
    """Biomass Luyssaert et. al. 2009 benchmark"""
    def __init__(self,config_name='config.ini',init=False,*args,**kwargs):
        ...
        #FAPAR parameters,
        Benchmark.import_config(self,config_name,'BiomassLuyssaert')
        #Load observation file and look for corresponding lat/lon coordinates in control file
        self.obs=pd.read_csv(self.fnobs,delim_whitespace=True,skiprows=7)
        #vegc=(biomass_ag+biomass_bg)*0.475 (Conversion factor for mass biomass to mass C in biomass)
        self.obs['vegc']=(self.obs['agb']+self.obs['bgb'])*0.475
    ...
    def get_sim(self,memberid):
        ...
        tconstraint=(vegc_cdf.TIME>1931.)*(vegc_cdf.TIME<1997.)
    ...
    def plot_member(self,memberid):
        "Plots given member and observed Luyssaert Biomass"
        #Plot setings
        cm='jet'
        levels=11
        ...
        ax.set_title('Simulated Vegetation Carbon ('+memberid+') vs Biomass Luyssaert')
        ax.coastlines()
        ...
        ax.set_ylabel(r'Vegetation Carbon observed (Biomass Luyssaert) [gC m$^2$ yr$^{-1}$]')
